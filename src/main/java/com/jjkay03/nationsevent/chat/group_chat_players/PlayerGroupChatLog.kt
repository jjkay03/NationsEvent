...
object PlayerGroupChatLog {
    ...
    private fun log(msg: String) {
        LogsManager.log(Saves.Companion.LOG_FILE_PLAYER_GC, "PGC", msg)
    }

    fun chatInGC(groupChat: PlayerGroupChat, player: OfflinePlayer?, message: String, staffAction: Boolean = false) {
        val author = when {
            player == null -> ""
            staffAction -> "${PlayerGroupChatManager.Companion.STAFF_MESSAGE_PREFIX_FORMATLESS} ${player.name}: "
            else -> "${player.name}: "
        }
    ...
    fun createGC(groupChat: PlayerGroupChat, staffAction: Boolean = false) {
        log("${if (staffAction) "${PlayerGroupChatManager.Companion.STAFF_MESSAGE_PREFIX_FORMATLESS} " else groupChat.owner.name} CREATED group chat ${groupChat.prefix}")
    }

    fun deleteGC(groupChat: PlayerGroupChat, staffAction: Boolean = false) {
        log("${if (staffAction) "${PlayerGroupChatManager.Companion.STAFF_MESSAGE_PREFIX_FORMATLESS} " else groupChat.owner.name} DELETED group chat ${groupChat.prefix}")
    }

    fun addPlayerToGC(groupChat: PlayerGroupChat, player: OfflinePlayer, staffAction: Boolean = false) {
        if (staffAction) { log("${PlayerGroupChatManager.Companion.STAFF_MESSAGE_PREFIX_FORMATLESS} ADDED ${player.name} to group chat ${groupChat.prefix}") }
    ...
    fun removePlayerFromGC(groupChat: PlayerGroupChat, player: OfflinePlayer, staffAction: Boolean = false) {
        log("${if (staffAction) "${PlayerGroupChatManager.Companion.STAFF_MESSAGE_PREFIX_FORMATLESS} " else groupChat.owner.name} REMOVED ${player.name} from group chat ${groupChat.prefix}")
    }
