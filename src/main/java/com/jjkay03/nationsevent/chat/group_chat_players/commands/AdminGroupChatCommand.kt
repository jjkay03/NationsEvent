...
class AdminGroupChatCommand : CommandExecutor, TabCompleter {
        ...
        when (args[0].lowercase()) {
            ...
            SubCommand.CREATE.cmd -> {
                ...
                if (!PlayerGroupChatUtils.validateGCName(args[1])) {
                    player.sendMessage("§c${PlayerGroupChatManager.Companion.PREFIX}Invalid group chat name, use only letters and max ${PlayerGroupChatManager.Companion.NAME_CHARACTER_LIMIT} characters!")
                    return true
                }
                ...
                if (groupChat == null) { player.sendMessage("§c${PlayerGroupChatManager.Companion.PREFIX}Failed to create group chat, ${targetPlayerOwner.name} has reached the group chat limit!"); return true }

                // Notify players
                val targetPlayersNames = groupChat.playerList.mapNotNull { it.name }.joinToString(", ")
                player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §aCREATED §7group chat %gc% §7with $targetPlayersNames", "§2", groupChat))
                val targetPlayersMembersMsg = PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You were §aADDED §7to group chat %gc% §7by staff", "§7", groupChat)
            ...
            SubCommand.DELETE.cmd -> {
                ...
                // Notify player
                player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §cDELETED §7group chat %gc%", "§c", groupChat))
            ...
            SubCommand.DELETEALL.cmd -> {
                ...
                for ( group in PlayerGroupChatManager.Companion.GROUP_CHATS.toList() ) {
                    // Notify player
                    player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §cDELETED §7group chat %gc%", "§c", group))
            ...
            SubCommand.ADD.cmd -> {
                ...
                player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §aADDED §7$targetNames to %gc%", "§7", groupChat))
                val targetPlayersMsg = PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You were §aADDED §7to group chat %gc% §7by staff", "§7", groupChat)
            ...
            SubCommand.REMOVE.cmd -> {
                ...
                // Notify players
                val targetNames = if (targetPlayers.size == 1) targetPlayers.first().name else targetPlayers.joinToString(", ") { it.name }
                player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §cREMOVED §7$targetNames to %gc%", "§7", groupChat))
                val targetPlayersMsg = PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You were §cREMOVED §7to group chat %gc% §7by staff", "§7", groupChat)
                Utils.sendMessageToPlayerList(targetPlayers, targetPlayersMsg)
            ...
            SubCommand.JOIN.cmd -> {
                ...
                if (groupChat.playerList.contains(player)) { player.sendMessage("§c${PlayerGroupChatManager.Companion.PREFIX}You are already a member of ${groupChat.prefix}!") }

                // Add player to group
                PlayerGroupChatUtils.addPlayerToGC(groupChat, listOf(player))

                // Alert player
                player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §aJOINED §7group chat %gc%", "§7", groupChat))
            ...
            SubCommand.LIST.cmd -> {
                ...
                when (args[1].lowercase()) {
                    ...
                    "@a", "all" -> {
                        ...
                        player.sendMessage("")
                        player.sendMessage("§e${PlayerGroupChatManager.Companion.PREFIX}All server group chats:")
                        player.sendMessage ("§7(Hover GC for more info")
                        for (group in PlayerGroupChatManager.Companion.GROUP_CHATS) {
                    ...
                    // List all groups a player is in
                        ...
                        player.sendMessage("")
                        player.sendMessage("§e${PlayerGroupChatManager.Companion.PREFIX}Group chats ${targetPlayer.name} is in:")
                        player.sendMessage ("§7(Hover GC for more info")
            ...
            SubCommand.SETOWNER.cmd -> {
                ...
                if (!groupChat.playerList.contains(targetPlayer)) { player.sendMessage("§c${PlayerGroupChatManager.Companion.PREFIX}${targetPlayer.name} is not a member of ${groupChat.prefix}!"); return true }
                if (targetPlayer == groupChat.owner) { player.sendMessage("§c${PlayerGroupChatManager.Companion.PREFIX}${targetPlayer.name} is already the owner of ${groupChat.prefix}!"); return true }

                // Set new owner
                PlayerGroupChatUtils.setGCOwner(groupChat, targetPlayer)

                // Notify players
                player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §bTRANSFERRED §7group chat %gc% §7to ${targetPlayer.name}", "§7", groupChat))
                targetPlayer.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}Group chat %gc% §7was §bTRANSFERRED §7to you by staff", "§7", groupChat))
            }
            ...
            SubCommand.SPY.cmd -> {
                ...
                when (args[1].lowercase()) {
                    ...
                    "clearall", "clear" -> {
                        ...
                        player.sendMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You CLEARED §dSPYING §7on all group chats")
                    }

                    // Spy on all
                    "@a", "all" -> {
                        // Check if player is global spy
                        if (!PlayerGroupChatManager.Companion.GLOBAL_SPIES.contains(player)) {
                            PlayerGroupChatManager.Companion.GLOBAL_SPIES.add(player)
                            player.sendMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §aENABLED §dGLOBAL SPYING §7on all group chats")
                        } else {
                            PlayerGroupChatManager.Companion.GLOBAL_SPIES.remove(player)
                            player.sendMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §cDISABLED §dGLOBAL SPYING §7on all group chats")
                        }
                    ...
                    // Spy on specific group
                        ...
                        if (PlayerGroupChatUtils.spyAdd(groupChat, player)) {
                            player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §aENABLED §dSPYING §7on group chat %gc%", "§7", groupChat))
                        } else {
                            PlayerGroupChatUtils.spyRemove(groupChat, player)
                            player.sendMessage(PlayerGroupChatUtils.formatHoverableMessage("§7${PlayerGroupChatManager.Companion.PREFIX}You §cDISABLED §dSPYING §7on group chat %gc%", "§7", groupChat))
                        }
                    }
                }
            }
        ...
        return when (args.size) {
            ...
            2 -> when (sub) {
                ...
                SubCommand.COORDS.cmd, SubCommand.CHAT.cmd, "c", SubCommand.DELETE.cmd, SubCommand.ADD.cmd, SubCommand.REMOVE.cmd, SubCommand.JOIN.cmd, SubCommand.SETOWNER.cmd ->
                    PlayerGroupChatUtils.tabCompletePlayerGCsList(PlayerGroupChatManager.Companion.GROUP_CHATS).filter { it.lowercase().startsWith(current) }

                // Show all group chats + @a + CLEAR
                SubCommand.SPY.cmd -> (listOf("@a", "CLEARALL") + PlayerGroupChatUtils.tabCompletePlayerGCsList(
                    PlayerGroupChatManager.Companion.GROUP_CHATS
    ...
    private fun getAndValidateGC(groupChatArgument: String, sender: CommandSender): PlayerGroupChat? {
        ...
        if (groupChat == null) { sender.sendMessage("§c${PlayerGroupChatManager.Companion.PREFIX}Invalid group chat ID!"); return null }
