...
object LuckPermsUtils {
    ...
    fun getAllGroups(): List<Group> {
        return NationsEvent.Companion.LP_GROUP_MANAGER.loadedGroups
    ...
    fun getGroup(name: String): Group? {
        return NationsEvent.Companion.LP_GROUP_MANAGER.getGroup(name)
    }
    ...
    fun groupGetAllUsers(group: Group?): List<UUID> {
        ...
        return NationsEvent.Companion.LP_USER_MANAGER.searchAll(NodeMatcher.key(InheritanceNode.builder(group.name).build()))
            .join()
            .map { it.key }
            .toList()
    }
    ...
    fun groupAddPermission(group: Group?, permission: String, value: Boolean = true): Boolean {
        ...
        group.data().add(node)
        NationsEvent.Companion.LP_GROUP_MANAGER.saveGroup(group)
        return true
    }
    ...
    fun playerGetPrimaryGroup(player: Player): Group? {
        val lpPlayer = NationsEvent.Companion.LP_USER_MANAGER.getUser(player.uniqueId) ?: return null
        val primaryGroupName = lpPlayer.primaryGroup
        return NationsEvent.Companion.LP_GROUP_MANAGER.getGroup(primaryGroupName)
    }

    // Function that give a player a permission
    fun playerAddPermission(player: Player, permission: String, value: Boolean = true) {
        val lpPlayer = NationsEvent.Companion.LP_USER_MANAGER.getUser(player.uniqueId) ?: return
        val node = Node.builder(permission).value(value).build()
        lpPlayer.data().add(node)
        NationsEvent.Companion.LP_USER_MANAGER.saveUser(lpPlayer)
    }
    ...
    fun isPlayerInGroup(player: Player, group: Group?): Boolean {
        val lpPlayer = NationsEvent.Companion.LP_USER_MANAGER.getUser(player.uniqueId) ?: return false
    ...
    fun playerPrefix(player: Player): String {
        val lpPlayer = NationsEvent.Companion.LP_USER_MANAGER.getUser(player.uniqueId)
